...
- The trigger point is `max(80% of the window, window - 16k)` — whichever leaves MORE room. The percentage keeps small windows safe (80% of 20k still leaves 4k, where a flat reserve would put the trigger at zero); the reserve stops large ones from wasting capacity (80% of 1M would interrupt with 200k still free). pi uses the reserve rule alone (16384); the percentage floor is the part iota adds for small windows.
