...
func TestExpand(t *testing.T) {
	...
	for in, want := range map[string]string{
		...
		"${chatchainHome}/sys.md":         filepath.Join(home, ".chatchain", "sys.md"),
		"${cwd}":                          wd,
		"${workspaceFolder}":              wd,
	...
	if !strings.HasSuffix(Expand("${chatchainHome}"), ".chatchain") {
		t.Error("chatchainHome must end in .chatchain")
	}
}
