...
// Len returns the number of items currently in scope (after any search filter).
func (l *List) Len() int {
	return len(l.scope)
}

// VisibleIndices returns the absolute indices (into the original Items slice) of
// the items currently visible in the window, in display order. It parallels
// Items: the i-th visible item's absolute index is VisibleIndices()[i]. Used by
// multi-select to map a rendered row back to its item for checkbox state.
func (l *List) VisibleIndices() []int {
	end := l.start + l.size
	if max := len(l.scope); end > max {
		end = max
	}
	pos := make(map[*interface{}]int, len(l.items))
	for i, item := range l.items {
		pos[item] = i
	}
	out := make([]int, 0, end-l.start)
	for i := l.start; i < end; i++ {
		out = append(out, pos[l.scope[i]])
	}
	return out
}
