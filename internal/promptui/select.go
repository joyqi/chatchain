...
import (
	...
	"chatchain/internal/promptui/list"
...
type Select struct {
	...
	// the chatchain/internal/readline#Config for more information on readline.
	IsVimMode bool
	...
	// multi-select state, set by RunMultiple. When multi is true each row is
	// prefixed with a checkbox and Space toggles the highlighted row; checked is
	// keyed by absolute item index.
	multi   bool
...
type Key struct {
	...
	// Check chatchain/internal/readline for a list of codes
	Code rune
...
// RunMultiple runs the select as a multi-select. Space toggles the highlighted
// row's checkbox (in place), Enter submits, and Ctrl+C cancels (callers wanting
// Esc-to-cancel should map Esc to Ctrl+C in their Stdin). It returns the checked
// rows' absolute indices in ascending order; on cancel it returns the underlying
// error (e.g. ErrInterrupt). HideSelected is forced on so the list clears on exit.
func (s *Select) RunMultiple() ([]int, error) {
	s.multi = true
...
func (s *Select) innerRun(cursorPos, scroll int, top rune) (int, string, error) {
	c := &readline.Config{
		Stdin:         s.Stdin,
		Stdout:        s.Stdout,
		EscapeCancels: true, // standalone ESC cancels the menu natively
	}
	err := c.Init()
	if err != nil {
		return 0, "", err
	...
	// Upstream promptui wraps c.Stdin in readline.NewCancelableStdin here; our
	// vendored readline cancels blocked stdin reads internally (terminal ioloop +
	// stopChan on Close), so the wrapper is unnecessary.
	...
	// q quits (like Ctrl+C / Esc) when not typing into a search box. Handled here
	// rather than in the listener because the listener can't end the run; it
	// shares searchMode with the listener closure below.
	...
	c.SetListener(func(line []rune, pos int, key rune) ([]rune, int, bool) {
		...
		case key == s.Keys.PageDown.Code || (key == 'l' && !searchMode):
			s.list.PageDown()
		case !searchMode && key == 'g':
			s.list.SetCursor(0) // jump to first
		case !searchMode && key == 'G':
			s.list.SetCursor(s.list.Len() - 1) // jump to last
		case s.multi && !searchMode && key == ' ':
			// Toggle the highlighted row's checkbox in place; the run continues.
			if i := s.list.Index(); i != list.NotFound {
		...
		for i, item := range items {
			...
			// In multi mode prepend a checkbox to the item so it renders inside the
			// template after the cursor (▸ [x] item), then let the active/inactive
			// template style the whole thing.
			rendered := item
			if s.multi {
				box := "[ ] "
				if i < len(absIdx) && s.checked[absIdx[i]] {
					box = "[x] "
				}
				rendered = box + fmt.Sprintf("%v", item)
			}

			output := []byte(page + " ")

			if i == idx {
				output = append(output, render(s.Templates.active, rendered)...)
			} else {
				output = append(output, render(s.Templates.inactive, rendered)...)
			}
...
func (s *Select) prepareTemplates() error {
	...
	if tpls.Help == "" {
		// Mode-aware one-liner: navigation always, multi-select keys only in
		// multi mode, search hint only when a Searcher is set. Callers no longer
		// need to cram key hints into the Label.
		tpls.Help = `{{ "↑/↓ navigate" | faint }}` +
			`{{ if .Multi }}{{ " · space select · enter confirm" | faint }}{{ end }}` +
			`{{ if .Search }}{{ " · / search" | faint }}{{ end }}` +
			`{{ " · esc/q quit" | faint }}`
	}
