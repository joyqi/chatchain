...
import (
	...
	"chatchain/internal/readline/internal/platform"
)
...
type Config struct {
	...
	// UniqueEditLine, when true, keeps a single editable line: readline does not
	// emit a trailing newline on Enter/EOF/Interrupt and cleans the line in place
	// instead. Used by promptui's Select, which renders the whole list itself and
	// only wants readline for in-place line editing. Re-added to this vendored
	// fork (the field existed in upstream chzyer/readline; ergochat dropped it).
	UniqueEditLine bool

	// EscapeCancels, when true, makes a standalone ESC keypress behave like Ctrl+C
	// (returns ErrInterrupt). A real escape sequence (arrow keys, etc.) arrives as
	// one burst so its bytes are already buffered after the ESC; a lone ESC leaves
	// nothing buffered, which is how the two are told apart. Used by promptui so
	// menus cancel on ESC without a stdin-rewriting wrapper.
	EscapeCancels bool
...
// Init initializes the config (filling defaults). It is idempotent — NewEx calls
// it internally — and exists for callers ported from chzyer/readline.
func (c *Config) Init() error {
	return c.init()
}

// SetListener sets the keypress listener. The instance reads Config.Listener live
// via the stored config pointer, so this takes effect even after NewEx.
func (c *Config) SetListener(f Listener) {
	c.Listener = f
}
