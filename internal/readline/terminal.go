...
import (
	...
	"chatchain/internal/readline/internal/ansi"
...
func (t *terminal) consumeANSIEscape(buf *bufio.Reader, ansiBuf *bytes.Buffer) (result readResult, err error) {
	ansiBuf.Reset()
	// A standalone ESC (no sequence bytes immediately following it) cancels when
	// EscapeCancels is set. A real escape sequence arrives as one burst, so its
	// trailing bytes are already buffered; a lone ESC leaves nothing buffered.
	if t.GetConfig().EscapeCancels && buf.Buffered() == 0 {
		return readResult{r: CharInterrupt, ok: true}, nil
	}
