...
import (
	...
	"chatchain/internal/readline/internal/runes"
)
...
func (o *operation) write(target io.Writer, b []byte) (int, error) {
	...
	uniqueEditLine := o.GetConfig().UniqueEditLine
	o.buf.Refresh(func() {
		n, err = target.Write(b)
		if uniqueEditLine {
			// promptui-style caller: it owns the screen and writes cursor-control
			// ANSI (cursor up/down, erase line, …) through here. Those bytes are
			// not plain text — ColorFilter doesn't strip cursor-movement escapes —
			// so recomputing the prompt column from them corrupts ppos and drifts
			// the render. The caller manages its own layout, so leave ppos alone.
			return
		}
	...
	for {
		...
		case CharEnter, CharCtrlJ:
			...
			if o.GetConfig().UniqueEditLine {
				// Single edit line: clean in place, no trailing newline.
				o.buf.Clean()
				data = o.buf.Reset()
			} else {
				o.buf.WriteRune('\n')
...
func (o *operation) getAndSetOffset(deadline chan struct{}) {
	...
	// promptui-style single edit line (UniqueEditLine): the caller manages the
	// screen with its own buffer and moves the cursor out of band, so a DSR
	// cursor-position query here races with those writes and yields a stale
	// offset (intermittent one-line drift + leftover residue on exit). Skip the
	// query and the edge-probe; render in place at the current cursor instead.
	if o.GetConfig().UniqueEditLine {
		o.buf.SetOffset(cursorPosition{1, 1})
		return
	}
