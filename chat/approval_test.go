...
func TestQuietLoopForwardsApproval(t *testing.T) {
	...
	host := quietHost{rec: newRunRecorder(),
		approve: func(_ context.Context, tc provider.ToolCall) (bool, string) {
...
func TestQuietHostAskApprovalShapes(t *testing.T) {
	var h quietHost
	ok, why := h.askApproval(context.Background(), provider.ToolCall{Name: "edit_file"})
	if ok || !strings.Contains(why, "edit_file") {
		t.Errorf("nil approver = (%v, %q)", ok, why)
	}
	h.approve = func(_ context.Context, tc provider.ToolCall) (bool, string) {
