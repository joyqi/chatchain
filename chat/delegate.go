...
type quietHost struct {
	...
	approve func(tc provider.ToolCall) (bool, string)
}

// askApproval resolves one gated call: allowed, or the refusal to hand back
// as the call's result.
func (h quietHost) askApproval(tc provider.ToolCall) (bool, string) {
	if h.approve == nil {
...
type Delegator struct {
	...
	mu      sync.Mutex
	approve func(agent string, tc provider.ToolCall) (bool, string)
}
...
func (d *Delegator) SetApprover(fn func(agent string, tc provider.ToolCall) (bool, string)) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.approve = fn
}

func (d *Delegator) approver() func(string, provider.ToolCall) (bool, string) {
	d.mu.Lock()
	defer d.mu.Unlock()
	return d.approve
}
...
func (d *Delegator) Run(ctx context.Context, spec tool.DelegateSpec) (tool.DelegateResult, error) {
	...
	if fn := d.approver(); fn != nil {
		...
		host.approve = func(tc provider.ToolCall) (bool, string) {
			d.mu.Lock()
			defer d.mu.Unlock()
			return fn(spec.Agent, tc)
		}
	}
